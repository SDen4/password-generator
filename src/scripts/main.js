...
doc.getElementById('strLength').onkeydown = function (e) {
  if (doc.getElementById('strLength').value) {
...
copyStrBtn.addEventListener('click', function () {
  doc
    .getSelection()
    .setBaseAndExtent(result, 0, result, result.childNodes.length);
...
function fn1() {
  let arrAddSymbols = [
    33, 64, 35, 36, 37, 94, 38, 42, 40, 41, 95, 43, 61, 45, 63, 58, 59, 8470,
    46, 44, 62, 124, 47, 93, 91, 123, 125,
  ];
  let ranAddSymb =
    arrAddSymbols[Math.floor(Math.random() * arrAddSymbols.length)];
